...
import yaml
import pandas as pd
from SystemC_oman_thermal_2 import ep_costs_func
...
def make_csv_and_plot(config_path, var_number):
    ...
    results_strings = (
        outputlib.views.convert_keys_to_strings(energysystem.results['main']))
    ...
    # Storage
    # charging_cycles_el =\
    #   (el_seq[(('electricity', 'storage_electricity'), 'flow')].sum()
    #    / none_scal[(('storage_electricity', 'None'), 'invest')])
    # charging_cycles_cool =
    #   (cool_seq[(('cool', 'storage_cool'), 'flow')].sum()
    #    / none_scal[(('storage_cool', 'None'), 'invest')])
    # charging_cycles_thermal =
    #   (thermal_seq[(('thermal', 'storage_thermal'), 'flow')].sum()
    #    / none_scal[(('storage_thermal', 'None'), 'invest')])

    # other components
    # lf_chil = (cool_seq[(('absorption_chiller', 'cool'), 'flow')].sum()
    #            / (cool_scal[(('absorption_chiller', 'cool'), 'invest')]
    #               * 8760))
    ...
    scalars_all.to_csv(
        csv_path + 'Oman_thermal_model_4_{0}_{1}_scalars.csv'.format(
            cfg['exp_number'], var_number))
    ...
    if var_number == (cfg['number_of_variations']-1):
        df_all_var.to_csv(
            csv_path
            + 'Oman_thermal_model_4_{0}_scalars_all_variations.csv'.format(
                cfg['exp_number']))
    ...
    sequences_df[('storage_thermal', 'None'), 'capacity'] = (
            none_res['sequences'][(('storage_thermal', 'None'), 'capacity')])
    sequences_df[('storage_cool', 'None'), 'capacity'] = (
            none_res['sequences'][(('storage_cool', 'None'), 'capacity')])

    sequences_df.to_csv(
        csv_path + 'Oman_thermal_model_4_{0}_{1}_sequences.csv'.format(
            cfg['exp_number'], var_number))
    ...
    oev.plot.set_datetime_ticks(ax_thermal, thermal_seq_resample.index,
                                tick_distance=14, date_format='%d-%m-%H',
                                offset=1)
    ...
    oev.plot.set_datetime_ticks(ax_cool, cool_seq_resample.index,
                                tick_distance=14, date_format='%d-%m-%H',
                                offset=1)
    ...
    oev.plot.set_datetime_ticks(ax_el, el_seq_resample.index,
                                tick_distance=14, date_format='%d-%m-%H',
                                offset=1)
    ...
    plt.savefig(
        plot_path + 'Oman_thermal_model_4_{0}_{1}.png'.format(
            cfg['exp_number'], var_number))
    ...
    csv_plot = pd.merge(csv_plot, el_seq_resample,
                        left_index=True, right_index=True)
    csv_plot.to_csv(
        plot_path + 'Oman_thermal_plot_model_4_{0}_{1}.csv'.format(
            cfg['exp_number'], var_number))
