...
Modeling a combined heat and power (CHP) plant with a battery to increase its flexibility.
...
storage_el = solph.components.GenericStorage(
    nominal_capacity=12.5,  # [MWh_el]
    label='storage_el',
    inputs={bel: solph.Flow(nominal_value=12.5, variable_costs=0)},  # [MW_el]
    outputs={bel: solph.Flow(nominal_value=12.5)},  # [MW_el]
    capacity_loss=0.001, initial_capacity=0,
    inflow_conversion_factor=1, outflow_conversion_factor=0.96)
...
if debug:
    filename = os.path.join(
        helpers.extend_basic_path('lp_files'), 'flexCHB_A1S.lp')
...
energysystem.dump(dpath="dumps", filename="flexCHB_A1S_dumps.oemof")

stop_time = timeit.default_timer()
run_time_in_sec = stop_time - start_time
print("***Run Time***")
if run_time_in_sec < 60:
    print("%6.2f" %run_time_in_sec, "s")
elif run_time_in_sec >= 60:
    run_time_in_min = run_time_in_sec/60
    residual_seconds = run_time_in_sec%60
    print("%6.0f" %run_time_in_min, "min,", "%6.2f" %residual_seconds, "s")
else:
    print("%12.2f" %run_time_in_sec, "s")
